...
class Ui_MainWindow(QtWidgets.QMainWindow):
    ...
    def btnStart_Click(self, neuronsPerLayer=10, alpha=1e-05, activationFunc=1, solver=1, maxIter = 1000, trainingSamples='func_10.csv'):
        ...
        mlp =  MLPRegressor(activation=activationFuncs[ACTIVATION_F], alpha=ALPHA, solver=solvers[SOLVER], hidden_layer_sizes=N_PER_LAYER,max_iter=MAX_ITER)
        ...
        #Learn neural network
            ...
            if trainingSamples == "func_10.csv":
                mlp.fit(learn_data_10[:,0].reshape(learn_data_10.shape[0],1), learn_data_10[:,1])
            elif trainingSamples == "func_30.csv":
                mlp.fit(learn_data_30[:,0].reshape(learn_data_30.shape[0],1), learn_data_30[:,1])
            elif trainingSamples == "func_60.csv":
                mlp.fit(learn_data_60[:,0].reshape(learn_data_60.shape[0],1), learn_data_60[:,1])
        ...
        #Calculate MSE
        self.mse = np.mean((test_data[:,1] - output)**2)
        print("MSE = ", self.mse)

    def autoRun(self):
        TRAINING_SAMPLES = ['func10.csv', 'func30.csv', 'func60.csv']
        NEURONS_PER_HIDDEN_LAYER = [5, 10, 30]
        HIDDEN_LAYERS = [1, 2, 3]
        ACTIVATION_FUNCS = [0, 1, 2, 3]
        TRAINING_ALGORITHM = [0, 1, 2]

        if not os.path.exists("data/"):
            os.makedirs("data/")
        if not os.path.exists("MSEValues/"):
            os.makedirs("MSEValues/")

        for sample in TRAINING_SAMPLES:
            for hiddenLayerSize in NEURONS_PER_HIDDEN_LAYER:
                for numberOfHiddenLayers in HIDDEN_LAYERS:
                    for activationFunc in ACTIVATION_FUNCS:
                        for trainingAlg in TRAINING_ALGORITHM:
                            successCnt = 0
                            bestMSE = 10000
                            bestLineChart = self.lineChart
                            MSEs = []
                            while successCnt < 5:
                                neursPerLayer = (hiddenLayerSize,) * numberOfHiddenLayers
                                self.btnStart_Click(neuronsPerLayer=neursPerLayer, trainingSamples=sample, activationFunc=activationFunc, solver=trainingAlg)
                                MSEs.append(self.mse)
                                if(self.mse < bestMSE):
                                    bestMSE = self.mse
                                    bestLineChart = self.lineChart
                                successCnt += 1
                            bestLineChart.print_jpg("data/TrainingSamples=" + sample + "&NeuronsPerHiddenLayer=" + str(neursPerLayer) + "&ActivationFunction=" + str(activationFunc) + "&TrainingAlg=" + str(trainingAlg) + ".png")
                            statsFName = "MSEValues/TrainingSamples=" + sample + "&NeuronsPerHiddenLayer=" + str(neursPerLayer) + "&ActivationFunction=" + str(activationFunc) + "&TrainingAlg=" + str(trainingAlg) + ".txt"
                            with open (statsFName, 'w') as dat:
                                dat.write("Best MSE: " + str(bestMSE) + "\nAll MSEs: " + str(MSEs))
