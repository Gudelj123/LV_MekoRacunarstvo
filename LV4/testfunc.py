...
class Ui_MainWindow(QtWidgets.QMainWindow):
    ...
    def btnStart_Click(self, functionExamples,nPerHiddenLayer,nOfHiddenLayers,activation_functs,solver_func):
        ...
        if (gui):
            ...
            N_PER_LAYER = functionExamples
            ACTIVATION_F = activation_functs
            SOLVER = solver_func
            MAX_ITER = 200
            
       #Create neural network
        mlp = MLPRegressor(hidden_layer_sizes=N_PER_LAYER, activation=activation_functs[ACTIVATION_F], solver=solver_func[SOLVER], alpha=ALPHA,
                    ...
                    for l in range(4):
                        for h in range(3):
                            ...
                            while(successCount<5):
                                self.btnStart_Click(functionExamples[i],nPerHiddenLayer[j],nOfHiddenLayers[k],activation_functs[l],solver_func[h])
                                if min(self.fits) == 0:
                                    successCount += 1
                                    genAvg += self.curr_g
                                    values = []
                     ...
                     graphDataFname = "graphData/popSize=" + str(populationSize[j]) + "_numElites=" + str(numOfElites[k]) + "_mutationRate=" + str(mutationRate[l]) + ".csv"
                     dataFrame = pandas.DataFrame(bestPathThroughGens)
                     dataFrame.to_csv(graphDataFname, index=True, header=False)
                     print(dataFrame.shape)
                     print(dataFrame)
    
                     statsfName = "stats/" + str(populationSize[j]) + "_popSize_" + str(numOfElites[k]) + "_elites_" + str(mutationRate[l]) + "_mutationRate_" + ".txt"
